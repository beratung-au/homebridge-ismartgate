...
function parseInfo(xml) {
  ...
  const knownModel = model && /^ismartgate\s*(LITE|PRO)?$/i.exec(model);
  if (model) result.model = knownModel ? 'iSmartGate' + (knownModel[1] ? ' ' + knownModel[1].toUpperCase() : '') : model;
  if (firmware && /^\d+(?:\.\d+){0,2}(?:[-+][A-Za-z0-9.-]+)?$/.test(firmware)) {
    // The LITE API reports 170 where the device app reports 1.7.0. Interpret
    // only the three-digit form on recognised iSmartGate controllers.
    // Dotted versions and other revision formats retain their original value.
    result.firmware = knownModel && /^[1-9]\d{2}$/.test(firmware) ? firmware.split('').join('.') : firmware;
  }
